...
void XmlStreamReader::parseEntryElement_QCResult_GradientDWICheck(const QDomElement & element, QTreeWidgetItem *parent)
{
  ...
  while( !child.isNull() )
    ...
    else if( child.toElement().tagName() == "value" || child.toElement().tagName() == "processing" ||
             child.toElement().tagName() == "green" || child.toElement().tagName() == "red" )
      {
...
void XmlStreamReader::parseValueElement_QCResult_GradientDWICheck(const QDomElement & element, QTreeWidgetItem *parent)
{
  ...
  else if( parametersQCResult_Gradient[parametersQCResult_Gradient.size() - 1].parameter.left(9) !=
  ...
  for( unsigned int i = 1; i < parametersQCResult_Gradient.size(); i++ )
    ...
    switch( s_mapQCRESULTStringValue[parametersQCResult_Gradient[i].parameter.toStdString()] )
      ...
      case DWI_SLICEWISECHECK:
        {
        QCRESULT->GetSliceWiseCheckProcessing().push_back(s_mapQCRESULTStringValue[parametersQCResult_Gradient[i].value
                                                                                   .toStdString()]);
        break;
        }
      // case DWI_INTERLACEWISECHECK:
      // {
      // IntWiseCk.InterlaceWiseCheckProcessing =
      // s_mapQCRESULTStringValue[parametersQCResult_Gradient[i].value.toStdString()];
      // break;
      // }
      case DWI_INTERLACEX:
        {
      ...
      // GrdWiseCheck.GradientWiseCheckProcessing =
      // s_mapQCRESULTStringValue[parametersQCResult_Gradient[i].value.toStdString()];
      // break;
      case DWI_GRADIENTX:
  ...
  for( unsigned int i = 0; i < paremeters.size(); i++ )
    ...
    if( paremeters[i].parameter.left(34) ==
      ...
      foreach(QString value, values)
        ...
        std::cout << "Test in JointDenoising Filter:" << protocol->GetDenoisingJointLMMSE().Filter_Radius[ik]
                  << std::endl;
        ik++;
        }
      }
...
void XmlStreamReader::readEntryElement_QCResult()
{
  ...
  while( !reader.atEnd() )
    ...
    if( reader.isStartElement() )
      ...
      else if( reader.name() == "value" || reader.name() == "processing" || reader.name() == "green" ||
               reader.name() == "red" )
        {
